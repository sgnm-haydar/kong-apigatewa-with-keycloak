...
curl -s -X POST http://localhost:8001/plugins \-d name=oidc \-d config.client_id=ifm_facility_client \-d config.client_secret=cBh97xovn2wC70DbnZfupiH4t1vQoDrU \-d config.bearer_only=yes \-d config.introspection_endpoint=http://172.30.99.121:8080/auth/realms/ifm/protocol/openid-connect/token/introspect \-d config.discovery=http://172.19.100.120:8080/auth/realms/ifm/.well-known/openid-configuration \ | python3 -mjson.tool
